...
def preprocessing_pipeline(
  ...
  multihost_gen = multihost_dataloading.MultiHostDataLoadIterator(dataloader, global_mesh, generate_padding_example)
...
def make_hf_train_iterator(
  ...
  if config.use_sft and config.use_multimodal:
    ...
    train_iter = preprocessing_pipeline(
        ...
        generate_padding_example=config.generate_padding_batch_train,
...
def make_hf_eval_iterator(
  ...
  if config.use_sft and config.use_multimodal:
    ...
    eval_iter = preprocessing_pipeline(
        ...
        add_eos=config.add_eos,
        packing=config.packing,
        generate_padding_example=config.generate_padding_batch_eval,
        use_dpo=config.use_dpo,
        use_sft=config.use_sft,
        sft_train_on_completion_only=config.sft_train_on_completion_only,
    )
